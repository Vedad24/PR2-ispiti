...
bool ValidirajLozinku(string sifra) {
	//[A-Za-z0-9!\"-#$%&/()=?*.,@]{7,}
	//(?=.*[0-9]{1,})(?=.*[a-z]{1,})(?=.*[A-Z]{1,})(?=.*[!\"-#$%&\/()=?*.,@]{1,})(?=.{7,})[a-zA-Z0-9!\"\-#$%&\/()=?*.,@]{7,}
	regex provjera("(?=.*[0-9]{1,})(?=.*[a-z]{1,})(?=.*[A-Z]{1,})(?=.*[!\"-#$%&\/()=?*.,@]{1,})[a-zA-Z0-9!\"\-#$%&\/()=?*.,@]{7,}");
	//regex provjera("(?=.*[0-9]{1,})(?=.*[a-z]{1,})(?=.*[A-Z]{1,})(?=.*[!\"-#$%&/()=?*.,@]{1,})(?=.{7,})");
	return(regex_search(sifra, provjera));
}

ostream& operator<<(ostream& COUT, Pojas poj) {
...
public:
	...
	void AddElement(T1 el1, T2 el2, int lok = -1) {
		if (*_trenutno >= max)
			throw exception("Nema vise prostora!");

		if (lok >= 0 && lok < max) {
			for (int i = *_trenutno; i > lok; i--)
			{
				_elementi1[i] = _elementi1[i - 1];
				_elementi2[i] = _elementi2[i - 1];
			}
			_elementi1[lok] = new T1(el1);
			_elementi2[lok] = new T2(el2);
		}
		else
		{
...
class Tehnika {
	...
	Kolekcija<int, Datum*, brojTehnika>* _ocjene = nullptr;
public:
	...
	bool operator==(const Tehnika& obj) {
		if (strcmp(obj._naziv, _naziv) != 0)
			return false;
		if (obj._ocjene->getTrenutno() != _ocjene->getTrenutno())
			return false;
		for (int i = 0; i < GetOcjene().getTrenutno(); i++)
		{
			if (GetOcjene().getElement1(i) != obj._ocjene->getElement1(i) ||
				GetOcjene().getElement2(i)->datToInt() != obj._ocjene->getElement2(i)->datToInt())
				return false;
		}
		return true;
	}
	...
	bool AddOcjena(int ocjena, Datum& dat) {
		if (_ocjene->getTrenutno() != 0 && abs(_ocjene->getElement2(_ocjene->getTrenutno() - 1)->datToInt() - dat.datToInt()) < 3)
		{
			return false;
		}

		_ocjene->AddElement(ocjena, new Datum(dat));
		return true;
	}
	...
	friend ostream& operator<< (ostream& COUT, const Tehnika& obj) {
		...
		COUT << "Prosjecna: " << obj.getPro() << endl;
...
public:
	...
	float getProsjecnaPolaganje() {
		float pro = 0.00f;
		if (_polozeneTehnike.size() == 0)
			return 0;
		for (int i = 0; i < _polozeneTehnike.size(); i++)
		{
			pro += _polozeneTehnike[i]->getPro();
		}
		return pro / _polozeneTehnike.size();
	}
...
public:
	...
	*/
	...
	bool AddTehniku(Pojas pojas, Tehnika& tehnika) {
		if (pojas > ZUTI) {
			...
			for (int i = 0; i < _polozeniPojasevi.size(); i++)
			{
				if (_polozeniPojasevi[i].GetPojas() == pojas - 1)
				{
					...
					{
						if (_polozeniPojasevi[i].GetTehnike()[j]->getPro() < 3.5)
							return false;
					}
				}
			}
		...
		Polaganje polaga(pojas);
		polaga.GetTehnike().push_back(new Tehnika(tehnika));
		_polozeniPojasevi.push_back(polaga);
	}
