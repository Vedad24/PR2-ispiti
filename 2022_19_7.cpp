...
public:
	Rjecnik(bool omoguciDupliranje = true) : _elementi1(nullptr),_elementi2(nullptr) {
	...
	//This could work
	//myb better way is possible

	Rjecnik<T1,T2> operator()(int _od, int _do) {
		/*Rjecnik temp = *this;
		int vel = _do - _od + 1;
		temp._trenutno = vel;
		temp._elementi1 = new T1[vel];
		temp._elementi2 = new T2[vel];
		int j = 0;
		for (int i = _od; i <= _do; i++)
		{
			temp._elementi1[j] = getElement1(i);
			temp._elementi2[j] = getElement2(i);
			j++;
		}
		return temp;*/
		...
		if (!_omoguciDupliranje) {
			for (int i = 0; i < _trenutno; i++)
			{
				if (_elementi1[i] == el1 && _elementi2[i] == el2)
					throw runtime_error("Nije moguce dupliranje elemenata!");
			}
		}
...
public:
	...
	friend ostream& operator<< (ostream& COUT, const ZadovoljstvoKupca& obj) {
		...
		{
			cout << obj._komentariKarakteristika.getElement1(i) << " - " <<
				obj._komentariKarakteristika.getElement2(i) << endl;
		}
		return COUT;
	}
	...
	void DodajKomentarKarakteristike(Karakteristike karakteristika, const char* komentar) {
		for (int i = 0; i < _komentariKarakteristika.getTrenutno(); i++)
		{
			if (_komentariKarakteristika.getElement1(i) == karakteristika)
				throw runtime_error("Vec komentarisana karakteristika!");
		}
...
public:
	...
	void DodajKupovinu(float km, ZadovoljstvoKupca& zadovoljstvo) {
	GetKupovine().AddElement(km, zadovoljstvo);
	if (km >= 10)
		_bodovi.push_back(km / 10);

	if(_bodovi.size() != 0){
		int zadnjiBodovi = _bodovi.at(_bodovi.size() - 1);

		if (zadnjiBodovi > 5) {
			this_thread::sleep_for(chrono::seconds(3));
			thread th1(&Kupac::posaljiMail, this, zadnjiBodovi);
			th1.join();
		}
	}
}
...
int main() {
	...
	*/
	...
	//cout << *dynamic_cast<Kupac*> (kupci[0]);
	/*cout << *kupci[1];
	cout << *kupci[2];*/
